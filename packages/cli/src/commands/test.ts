...
function checkPluginInterface(dir: string, main: string): string[] {
  ...
  if (
    ...
    errors.push(
      "No default export found. Extensions must export a default class or object.",
    );
  }
  ...
  for (const method of REQUIRED_METHODS) {
    ...
    const methodRegex = new RegExp(`(?:async\\s+)?${method}\\s*\\(`, "m");
...
export async function testCommand(options: TestOptions = {}): Promise<void> {
  ...
  if (result.manifest) {
    log.info("Validating against the canonical Volt extension JSON Schema...");
    try {
